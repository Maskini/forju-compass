...
const labels = {
  ...
  outdated_information: "Veraltete Information",
  other: "Sonstiges",
};
...
export function feedbackEmailConfig(env = process.env) {
  ...
  const validFrom = from && z.email().safeParse(from).success;
  const validTo = to && z.email().safeParse(to).success;

  if (!key || !validFrom || !validTo) {
    throw new FeedbackConfigurationError(
      "Configure RESEND_API_KEY, FEEDBACK_FROM_EMAIL and FEEDBACK_TO_EMAIL in the server environment."
    );
  }
...
export function feedbackEmail(report: FeedbackReport) {
  const rows: [string, string][] = [
    ["Art des Problems", labels[report.type]],
    ...
    ["Seitentitel", report.pageTitle || "Nicht angegeben"],
    ...
    ...(report.conversationId
      ? [["Conversation ID", report.conversationId] as [string, string]]
      : []),
    ...(report.sessionId
      ? [["Session ID", report.sessionId] as [string, string]]
      : []),
  ];
  ...
  return {
    ...
    html: `<!doctype html><html lang="de"><body style="margin:0;background:#fffdfb;font-family:Arial,sans-serif;color:#2d1e1e"><main style="max-width:640px;margin:24px auto;padding:28px;border:1px solid #eadedc;border-radius:20px"><p style="color:#861456;font-weight:bold">ForJu Compass</p><h1 style="font-size:24px">Neue Rückmeldung</h1>${rows
      .map(
        ([label, value]) =>
          `<h2 style="font-size:14px;margin:24px 0 6px;color:#6b5a5a">${escapeEmailHtml(
            label
          )}</h2><p style="margin:0;white-space:pre-wrap;overflow-wrap:anywhere">${escapeEmailHtml(
            value
          )}</p>`
      )
      .join("")}</main></body></html>`,
...
export async function sendFeedbackEmail(
  ...
  try {
      ...
      resend.emails.send(
        {
          ...
          ...feedbackEmail(report),
        },
        { idempotencyKey }
      ),
      new Promise<never>((_, reject) => {
        timer = setTimeout(
          () => reject(new FeedbackDeliveryError("provider_timeout")),
          15000
        );
      }),
    ]);
    ...
    if (result.error || !result.data?.id) {
      throw new FeedbackDeliveryError("provider_rejected");
    }
